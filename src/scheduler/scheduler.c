...
void Scheduler__cpu_run(){
    ...
    while (1) {
        ...
        if (kernel->scheduler->sched_proc == NULL){
            ...
            flag = Scheduler__exec_process(segment, kernel->scheduler->sched_proc, kernel->scheduler->quantum, page->instr_per_page);
...
int Scheduler__exec_process(Segment *seg, Process *proc, int quantum, int instr_per_page) {
    ...
    for (p = Memory__fetch_page(seg, page_id); (p != NULL) && (quantum > 0); p = p->next) {
        ...
        for (i = page->instructions->head; (i != NULL) && (quantum > 0) ; i = i->next) {
